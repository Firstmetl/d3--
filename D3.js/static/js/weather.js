...
var svg1 = d3.select("#temp-map").append("svg")
    .attr("width", width)
    .attr("height", height)
    .append("g")
...
d3.json("./js/china.json", function(error, root) {
    ...
    svg1.selectAll("text")
        ...
        .attr("transform", function(d, i) {
            ...
            {
                return "translate(" + (path1.centroid(d)[0]) + "," + (path1.centroid(d)[1]) + ")";
            }
            return "translate(" + (path1.centroid(d)[0] - 10) + "," + path1.centroid(d)[1] + ")";
        })
    ...
    d3.csv("./scv/weather1.csv", function(error, dataset) {
        console.log(dataset)
        ...
        svg1.selectAll('path').attr('fill', function(d, i) {
            ...
            .on("mouseover", function(d, i) {
                ...
                tooltip1.html(d.properties.name + ":" + datas[d.properties.name])
        ...
        //数据初值
        var startText = svg1.append("text")
            .attr("x", 50)
            .attr("y", 45)
            .text(mindata);
        //数据中间值
        var middleText = svg1.append("text")
            .attr("x", 125)
            .attr("y", 45)
            .text(
                0 //中间值
        ...
        //数据末值
        var endText = svg1.append("text")
            .attr("x", 250)
            .attr("y", 45)
            .text(maxdata);
        var tickText = svg1.append('text')
            .attr('x', 50)
            .attr('y', 85)
            .attr('fill', 'green')
            .text('全国平均温度')
