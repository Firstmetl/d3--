...
var svg3 = d3.select("#rengonglin-map").append("svg")
    .attr("width", width)
    .attr("height", height)
    .append("g")
...
d3.json("../js/china.json", function(error, root) {
    ...
    svg3.selectAll("text")
        ...
        .attr("transform", function(d, i) {
            ...
            {
                return "translate(" + (path3.centroid(d)[0]) + "," + (path3.centroid(d)[1]) + ")";
            }
            return "translate(" + (path3.centroid(d)[0] - 10) + "," + path3.centroid(d)[1] + ")";
        })
    ...
    d3.csv("../scv/人工林面积.csv", function(error, dataset) {
        console.log(dataset)
        ...
        svg3.selectAll('path').attr('fill', function(d, i) {
            ...
            .on("mouseover", function(d, i) {
                ...
                tooltip3.html(d.properties.name + ":" + datas[d.properties.name])
        ...
        var colorRect1 = svg3.append("rect")
            //x,y 矩形的左上角坐标
            .attr("x", 50)
            .attr("y", 50)
            //矩形的宽高
        ...
        //数据初值
        var startText = svg3.append("text")
            .attr("x", 50)
            .attr("y", 45)
            .text(mindata);
        //数据中间值
        var middleText = svg3.append("text")
            .attr("x", 125)
            .attr("y", 45)
            .text(
                d3.median(datam) //中间值
        ...
        //数据末值
        var endText = svg3.append("text")
            .attr("x", 250)
            .attr("y", 45)
            .text(maxdata);
        var tickText = svg3.append('text')
            .attr('x', 50)
            .attr('y', 85)
            .attr('fill', 'green')
            .text('森林面积')
